...
module.exports = {
	init: async(data) => {
		if (!data) { throw new Error("You must provide initialization values"); }
	...
	getTokenBalance: async(address) => {
		const contract = new config.web3.eth.Contract(config.token.abi, config.token.contract_address);
